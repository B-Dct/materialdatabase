...
import type { Assembly, MaterialProperty, Measurement } from '@/types/domain';
...
export function AssemblyPropertiesView({ assembly, measurements }: AssemblyPropertiesViewProps) {
    const { requirementProfiles, specifications, properties: globalProperties } = useAppStore();

    // 1. Data Prep
    const assignedProfiles = useMemo(() => {
        return requirementProfiles.filter(p => (assembly.assignedProfileIds || []).includes(p.id));
    }, [requirementProfiles, assembly.assignedProfileIds]);

    const assemblySpecs = useMemo(() => {
        return specifications.filter(s => s.assemblyId === assembly.id);
    }, [specifications, assembly.id]);
    ...
    useEffect(() => {
        const allIds = [
            ...
            ...assemblySpecs.map(s => s.id)
        ];
        if (visibleColumnIds.length === 0 && allIds.length > 0) {
            setVisibleColumnIds(allIds);
        }
    }, [assignedProfiles.length, assemblySpecs.length]);


    // 3. Filtered Measurements (Stats)
    const filteredMeasurements = useMemo(() => {
        // Filter measurements relevant to this ASSEMBLY?
    ...
    const matrixRows = useMemo(() => {
        ...
        const getRow = (name: string, unit: string, method: string = "") => {
            ...
            const key = `${normalizedName}|${normalizedMethodKey}`;

            if (!rows.has(key)) {
                // Find prop def by name to correct casing if needed, or just use name
        ...
        // B. Process Specification Values (Assembly Properties)
        // Only include properties that are LINKED to a specification
        (assembly.properties || []).filter(p => !!p.specificationId).forEach(p => {
    ...
    }, [assignedProfiles, assemblySpecs, assembly.properties, globalProperties, visibleColumnIds, statsMap]);

    // Columns Definition for UI
    const visibleProfiles = assignedProfiles.filter(p => visibleColumnIds.includes(p.id));
    const visibleSpecs = assemblySpecs.filter(s => visibleColumnIds.includes(s.id));
                ...
                <div className="flex items-center gap-2">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="start" className="w-[300px]">
                            ...
                            {assignedProfiles.length === 0 && assemblySpecs.length === 0 && (
