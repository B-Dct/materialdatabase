...
interface AssemblyUsageProps {
    assembly: Assembly;
}

export function AssemblyUsage({ assembly }: AssemblyUsageProps) {
    ...
    useEffect(() => {
        ...
        const fetchUsage = async () => {
            setIsLoading(true);
            try {
                const data = await storage.getAssemblyProjectUsage(assembly.id);
                if (isMounted) {
                    setRecords(data);
                }
            } catch (error) {
                console.error("Failed to load assembly project usage", error);
            } finally {
                if (isMounted) {
                    setIsLoading(false);
                }
    ...
    if (records.length === 0) {
        return (
            <div className="py-12 text-center border-2 border-dashed rounded-lg bg-muted/10 text-muted-foreground">
                ...
                <p className="text-sm">This assembly is not currently assigned to any projects.</p>
            </div>
        );
    }
