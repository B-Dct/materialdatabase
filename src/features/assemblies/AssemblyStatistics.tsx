import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import type { Assembly } from "@/types/domain";
import { Activity, ClipboardCheck, Calendar, FlaskConical, FileCheck } from "lucide-react";

interface AssemblyStatisticsProps {
    assembly: Assembly;
}

export function AssemblyStatistics({ assembly }: AssemblyStatisticsProps) {
    const standardCount = assembly.assignedProfileIds?.length || 0;
    const propertyCount = assembly.properties?.length || 0;
    const specifiedProperties = assembly.properties?.filter(p => p.specification).length || 0;
    const activeMeasurements = (assembly.measurements || []).filter(m => m.isActive !== false);
    const testCount = activeMeasurements.length;

    const lastTestDate = activeMeasurements.length
        ? new Date(Math.max(...activeMeasurements.map(m => new Date(m.date).getTime()))).toLocaleDateString()
        : 'N/A';

    const creationDate = assembly.createdAt ? new Date(assembly.createdAt) : new Date();
    const daysSinceCreation = Math.floor((new Date().getTime() - creationDate.getTime()) / (1000 * 3600 * 24));

    return (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Defined Properties</CardTitle>
                    <ClipboardCheck className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">{propertyCount}</div>
                    <p className="text-xs text-muted-foreground">{specifiedProperties} with specific context</p>
                </CardContent>
            </Card>

            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Assigned Standards</CardTitle>
                    <FileCheck className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">{standardCount}</div>
                    <p className="text-xs text-muted-foreground">Compliance profiles</p>
                </CardContent>
            </Card>

            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Tests Performed</CardTitle>
                    <FlaskConical className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">{testCount}</div>
                    <p className="text-xs text-muted-foreground">Last: {lastTestDate}</p>
                </CardContent>
            </Card>

            <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Assembly Age</CardTitle>
                    <Calendar className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">{daysSinceCreation} Days</div>
                    <p className="text-xs text-muted-foreground">Created {creationDate.toLocaleDateString()}</p>
                </CardContent>
            </Card>

            <Card className="col-span-full md:col-span-2">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Maturity Score</CardTitle>
                    <Activity className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                    <div className="text-2xl font-bold">
                        {Math.min(100, (propertyCount * 10) + (testCount * 5))}%
                    </div>
                    <div className="h-2 w-full bg-secondary mt-2 rounded-full overflow-hidden">
                        <div
                            className="h-full bg-primary transition-all"
                            style={{ width: `${Math.min(100, (propertyCount * 10) + (testCount * 5))}%` }}
                        />
                    </div>
                    <p className="text-xs text-muted-foreground mt-2">
                        Calculated based on data completeness.
                    </p>
                </CardContent>
            </Card>
        </div>
    );
}
